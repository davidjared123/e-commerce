...
const Cart = ({ cart, handleUpdateCart, handleRemoveCart, handleEmptyCart })=> {
    ...
    const EmptyCart = () => (
        <Typography variant='subtitle1'>No tienes ningun item añadido a la lista de compra,  
            <Link to='/' className={classes.link}> añade a menos 1 item!</Link>
        </Typography>
    )
            ...
            <Grid container spacing={3}>
                {
                    cart.line_items.map((item)=>(
                        <Grid item xs={12} sm={4} key={item.id}>
                            <CartItem item={item} 
                            onUpdatedCart={handleUpdateCart} 
